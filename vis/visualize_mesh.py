...
if __name__ == "__main__":
    obj_file_path = "./object.obj"
    vertices, faces = load_obj(obj_file_path)

    with open('../config/poses/example_pose.yaml', 'r') as yaml_file:
        desired_pose = yaml.load(yaml_file, Loader=yaml.FullLoader)

    translation = desired_pose['pose']['translation']
    rotation = desired_pose['pose']['rotation']
    ...
    vertices = np.dot(np.hstack((vertices, np.ones((vertices.shape[0], 1)))), trans.T)[:, :3]
    visualize_mesh(vertices, faces)
    # Set the desired height of the plane
    h = 0.05  # Change this to your desired height
